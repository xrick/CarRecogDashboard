...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        conn_group = QGroupBox("連線參數（依 PDF 規範 4.1.1）")
        row1 = QHBoxLayout()
        ...
        row1.addWidget(self.user_edit, 1)
        row1.addWidget(QLabel("Pwd:"))
        row1.addWidget(self.pwd_edit, 1)
        row1.addWidget(QLabel("碼流:"))
        row1.addWidget(self.subtype_combo)
        conn_group.setLayout(row1)
    ...
    def on_connect_all(self):
        ...
        for i in range(NUM_STREAMS):
            channel = i + 1                       # channel 從 1 開始
            url = build_rtsp_url(host, port, user, pwd, channel, subtype)

            tile = self.tiles[i]
            tile.set_title(f"CH {channel}  (sub={subtype})")
            tile.set_status("啟動中…")
        ...
        self.statusBar().showMessage(f"已啟動 {NUM_STREAMS} 路串流")
    ...
    def on_toggle_record_all(self):
        if not self._recording_all:
            ...
            for wid, worker in self.workers.items():
                path = os.path.join(folder, f"ch{wid + 1:02d}_{ts}.mp4")
